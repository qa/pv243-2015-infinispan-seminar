 ...
 */
package cz.muni.fi.pv243.seminar.infinispan.jsf;
...
import cz.muni.fi.pv243.seminar.infinispan.model.Car.CarType;
import cz.muni.fi.pv243.seminar.infinispan.model.Car.Country;
import cz.muni.fi.pv243.seminar.infinispan.session.CacheContainerProvider;
import cz.muni.fi.pv243.seminar.infinispan.session.CarManager;

/**
 * Populates a cache with initial data.
 * We need to obtain BeanManager from JNDI and create an instance of CacheContainerProvider
...
public class PopulateCache implements SystemEventListener {
    ...
    private BeanManager getBeanManagerFromJNDI() {
        ...
        try {
            ...
            result = context.lookup("java:comp/BeanManager"); // lookup in WildFly
    ...
    private UserTransaction getUserTransactionFromJNDI() {
        ...
        try {
            context = new InitialContext();
            result = context.lookup("java:comp/UserTransaction"); // lookup in WildFly
